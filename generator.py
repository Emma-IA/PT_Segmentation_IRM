...
class DataGeneratorClassifier(tf.keras.utils.Sequence):
    ...
    def __data_generation(self, list_IDs_temp, list_labels_temp):
        ...
        Y = np.round(np.empty((self.batch_size, *self.image_size), dtype=int) * 10**(-50),0)
        ...
        for i in range(len(list_IDs_temp)):
          ...
          label_redim = np.asarray(np.reshape(label_redim,(self.image_size[2],self.image_size[0],self.image_size[1])))
          # label_redim = np.asarray(np.argmax(label_redim,3))
        ...
        return X, tf.one_hot(Y.astype(np.int32), NBR_CLASSES)
